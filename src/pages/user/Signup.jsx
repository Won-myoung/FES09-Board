...
function Signup() {
  ...
  const onSubmit = async (formData) => {
    ...
    } catch (err) { // AxiosError(네트워크 에러-response가 없음, 서버의 4xx, 5xx 응답 상태 코드를 받았을땐-response 있음)
      if(err.response?.data.errors){
        err.esponse?.data.erros.forEach(error => setError(error.path,{ message: error.msg }));
...
<div className="min-w-80 flex-grow flex items-center justify-center">
      <div className="p-8 shadow-md rounded-lg w-full max-w-md dark:bg-gray-600">
        ...
        <form onSubmit={ handleSubmit(onSubmit) }>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 dark:text-gray-200 font-bold mb-2" htmlFor="profileImage">이미지</label>
            <input 
              type="file" 
              accept="image/*"
              id="profileImage" 
